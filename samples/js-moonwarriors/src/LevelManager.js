...
define([
    ...
    var LevelManager = cc.Class.extend({
        ...
        addEnemyToGameLayer:function(enemyType){
            ...
            addEnemy.x = 80 + (cc.winSize.width - 160) * Math.random();
            addEnemy.y = cc.winSize.height;
            ...
            switch (addEnemy.moveType) {
                case MW.ENEMY_MOVE_TYPE.ATTACK:
                    ...
                    tmpAction = actions.moveTo(1, offset);
                    break;
                case MW.ENEMY_MOVE_TYPE.VERTICAL:
                    offset = cc.p(0, -cc.winSize.height - addEnemy.height);
                    tmpAction = actions.moveBy(4, offset);
                    break;
                case MW.ENEMY_MOVE_TYPE.HORIZONTAL:
                    offset = cc.p(0, -100 - 200 * Math.random());
                    a0 = actions.moveBy(0.5, offset);
                    a1 = actions.moveBy(1, cc.p(-50 - 100 * Math.random(), 0));
                    var onComplete = actions.callFunc(function (pSender) {
                        var a2 = actions.delayTime(1);
                        var a3 = actions.moveBy(1, cc.p(100 + 100 * Math.random(), 0));
                        pSender.runAction(actions.sequence(a2, a3, a2.clone(), a3.reverse()).repeatForever());
                    }.bind(addEnemy) );
                    tmpAction = actions.sequence(a0, a1, onComplete);
                    break;
                case MW.ENEMY_MOVE_TYPE.OVERLAP:
                    var newX = (addEnemy.x <= cc.winSize.width / 2) ? 320 : -320;
                    a0 = actions.moveBy(4, cc.p(newX, -240));
                    a1 = actions.moveBy(4,cc.p(-newX,-320));
                    tmpAction = actions.sequence(a0,a1);
                    break;
            }
